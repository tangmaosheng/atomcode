 ...
 * @filesource /system/core/model.php
 * 
 */
 ...
 * @subpackage  Core
 */
class Model extends ac_base implements IteratorAggregate
{
	// 数据库连接对象列表
	protected $_db = array();
	...
	public function __construct($data='')
	{
		...
		if(!$this->composite) 
		{
			//$me = load_class('Db',1,'databases');
			include SYS_PATH . '/databases/Db.class.php';
			...
			// 设置数据库的返回数据格式
			$this->db->resultType   =   $this->config['DATA_RESULT_TYPE'];
			//为获得ORACLE自增LastID而统一考虑的
			$this->db->tableName = $this->parseName($this->name);
			// 设置默认的数据库连接
			$this->_db[0]   =   &$this->db;
			// 设置表前后缀
			$this->tablePrefix = $this->tablePrefix ? $this->tablePrefix : $this->config['DB']['PREFIX'];
			$this->tableSuffix = $this->tableSuffix ? $this->tableSuffix : $this->config['DB']['SUFFIX'];
			// 数据表字段检测
	...
	// Query回调方法
	protected function _before_query(&$sql) 
	{
		return true;
	}
	...
	private function _checkTableInfo()
	{
		...
		{
			// 如果数据表字段没有定义则自动获取
			if($this->config['DB']['FIELDS_CACHE'])
			{
				/**
				 * @todo fast cache
				 */
	...
	public function rsToVo($resultSet, $returnList=false, $position=0, $relation='')
	{
		if($resultSet ) 
		{
			if(!$returnList) 
			{
				...
				// 对数据对象自动编码转换
				$result  =   auto_charset($result, $this->config['DB']['CHARSET'], $this->config['CHARSET']);
				// 记录当前数据对象
			...
			else
			{
				...
				// 对数据集对象自动编码转换
				$resultSet  =   auto_charset($resultSet, $this->config['DB']['CHARSET'], $this->config['CHARSET']);
				// 记录数据列表
	...
	private function _createData($data) 
	{
		...
		if(!$this->composite && isset($data[$this->getPk()])) 
		{
			...
			if($rs && count($rs)>0) 
			{
				$type	=   'edit';
				$vo = $rs[0];
				if(DATA_TYPE_OBJ == $this->config['DATA_RESULT_TYPE']) 
				{
					// 对象模式
	...
	private function autoValidation($data, $type) 
	{
		...
		if(!empty($this->_validate)) 
		{
			// 如果设置了数据自动验证
			// 则进行数据验证
			load_class('validation',false);
			// 是否多字段验证
			$multiValidate  =   $this->config['MULTI_FIELD_VALIDATE'];
			// 重置验证错误信息
	...
	public function getModelName()
	{
		if(empty($this->name)) 
		{
			$prefix =   $this->config['MODEL_CLASS_PREFIX'];
			$suffix =   $this->config['MODEL_CLASS_SUFFIX'];
			if(strlen($suffix)>0) 
			{
	...
	public function getTableName()
	{
		if(empty($this->trueTableName)) 
		{
			...
			else
			{
				...
				elseif($this->config['TABLE_NAME_IDENTIFY'])
				{
					// 智能识别表名
