...
Class DbIbase extends Db{
	...
	public function __construct($config='')
	{
		...
		{
			throw new Exception(L('_NOT_SUPPERT_').':Interbase or Firebird');
		}
		if(!empty($config)) 
		{
			$this->Config   =   $config;
	...
	public function connect($config='',$linkNum=0) 
	{
		if ( !isset($this->linkID[$linkNum]) ) 
		{
			...
			$this->linkID[$linkNum] = $conn($host.':'.$config['database'], $config['username'], $config['password'],$this->config['DB']['CHARSET'],0,3);
