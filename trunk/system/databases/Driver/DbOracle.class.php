...
Class DbOracle extends Db{
	...
	public function __construct($config='')
	{
		...
		{
			throw new Exception(L('_NOT_SUPPERT_').'oracle');
		}
		if(!empty($config)) 
		{
			$this->Config		=		$config;
	...
	protected function _query($str='') 
	{
		...
		if (!$this->autoCommit)
		{
				if($this->isMainIps($this->queryStr)) $this->startTrans();
		}
		else 
		{
				//更改事务模式
	...
	public function close() 
	{
		...
		{
			throw new Exception($this->error(false));
		}
		$this->_linkID = 0;
	}
	...
	public function insert_last_id()
	{
		...
		$sequenceName = $this->config["DB"]["SEQUENCE_PREFIX"] . $this->tableName;
