...
class DbActiveRecord extends DbDriver {
	...
	function order_by($orderby, $direction = '') {
		if (strtolower($direction) == 'random') {
			...
			$direction = (in_array(strtoupper(trim($direction)), array('ASC', 
				'DESC'), TRUE)) ? ' ' . $direction : ' ASC';
		}
	...
	function _compile_select($select_override = FALSE) {
		...
		if (count($this->ar_from) > 0) {
			$sql .= "\nFROM ";
			$this->ar_from = array_unique($this->ar_from);
			$this->ar_from = array_unique($this->ar_from);
	...
	function flush_cache() {
		$this->_reset_run(array('ar_cache_select' => array(), 
			'ar_cache_from' => array(), 'ar_cache_join' => array(), 
			'ar_cache_where' => array(), 'ar_cache_like' => array(), 
			'ar_cache_groupby' => array(), 'ar_cache_having' => array(), 
			'ar_cache_orderby' => array(), 'ar_cache_set' => array(), 
	...
	function _reset_run($ar_reset_items) {
		foreach ($ar_reset_items as $item => $default_value) {
			if (array_key_exists($item, $this->ar_store_array)) {
				unset($this->ar_store_array[$item]);
			} else {
	...
	function _reset_select() {
		$ar_reset_items = array('ar_select' => array(), 'ar_from' => array(), 
			...
			'ar_distinct' => FALSE, 'ar_limit' => FALSE, 'ar_offset' => FALSE, 
			'ar_order' => FALSE);
	...
	public function keep_where($keep = 1) {
		if ($keep) {
			$this->ar_store_array['ar_where'] = $keep;
			$this->ar_store_array['ar_like'] = $keep;
			$this->ar_store_array['ar_orderby'] = $keep;
			$this->ar_store_array['ar_wherein'] = $keep;
			$this->ar_store_array['ar_join'] = $keep;
		}
	}
}
