...
class DbDriver {
	...
	protected $autoinit = TRUE; // Whether to automatically initialize the DB
	...
	private function initialize() {
		...
		if ($this->name != '') {
			if (!$this->db_select()) {
				log_message('error', 'Unable to select database: ' . $this->name);
				
				if ($this->db_debug) {
					$this->display_error('db_unable_to_select', $this->name);
				}
				return FALSE;
			} else {
				// We've selected the DB. Now we set the character set
				if (!$this->db_set_charset($this->charset, $this->dbcollat)) {
					return FALSE;
				}
	...
	function db_set_charset($charset, $collation) {
		if (!$this->_db_set_charset($this->charset, $this->dbcollat)) {
			log_message('error', 'Unable to set database connection charset: ' . $this->charset);
			
			if ($this->db_debug) {
				$this->display_error('db_unable_to_set_charset', $this->charset);
			}
	...
	function query($sql, $binds = FALSE, $return_object = TRUE) {
		...
		if (($this->table_prefix != '' and $this->swap_pre != '') and ($this->table_prefix != $this->swap_pre)) {
			$sql = preg_replace("/(\\W)" . $this->swap_pre . "(\\S+?)/", "\\1" . $this->table_prefix . "\\2", $sql);
		}
		...
		return $this->wrap_result();
	}
	
	function setReturnType($type) {
		if (in_array($type, array(MYSQL_ASSOC, MYSQL_BOTH, MYSQL_NUM))) $this->returnType = $type;
	}
	
	function wrap_result() {
		$result = array();
		while (($row = mysql_fetch_array($this->result_id, $this->returnType)) !== FALSE) {
			$result[] = $row;
		}
	...
	/**
	 * Returns all the queries you has executed
	 * 
	 * @return Array
	 */
	...
	function call_function($function) {
		$driver = ($this->type == 'postgre') ? 'pg_' : $this->type . '_';
	...
	function _cache_init() {
		...
		// @todo 加入缓存机制
		if (is_object($this->CACHE) and class_exists('DbCache')) {
			return TRUE;
		}
	 ...
	 *
	 * SELECT * FROM host.database.table.column AS c FROM host.database.table
	 *
	...
	function _protect_identifiers($item, $prefix_single = FALSE, $protect_identifiers = NULL, $field_exists = TRUE) {
		...
		if (strpos($item, '.') !== FALSE) {
			...
			if ($this->table_prefix != '') {
				...
				// Do we have 4 segments (host.database.table.column)?
...
// If so, we add the table prefix to the column name in 1st segment
				...
				if (substr($parts[$i], 0, strlen($this->table_prefix)) != $this->table_prefix) {
		...
		if ($this->table_prefix != '') {
			...
			if ($prefix_single == TRUE and substr($item, 0, strlen($this->table_prefix)) != $this->table_prefix) {
...
/* Location: ./system/library/DbDriver.php */