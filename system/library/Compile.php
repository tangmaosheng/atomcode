...
class Compile 
{
	...
	public function parseFile()
	{
		...
		{
			throw new CompileException('View file have not been given.');
		}
		
		if (!file_exists($this->sourceFile))
		{
			throw new CompileException('Missing view file, can not compile it:' . $this->sourceFile);
		}
		...
		{
			throw new CompileException('You may have no privilege to read this view:' . $this->sourceFile);
		}
	...
	private function parseInclude($depth, $reference, $file)
	{
		if ($file{0} == '/')
		{
			$path = APP_PATH . '/view' . $file . $this->viewExt;
		}
		else
		{
		...
		if (strpos(str_replace('\\', '/', $path), APP_PATH . '/view/') !== 0)
		{
			throw new CompileException('It\'s not allowed to include foreign file, or this file does not exist:' . $file . $this->viewExt);
		}
		
		if (!file_exists($path))
		{
			throw new CompileException('Included file is not existed : ' . $path . ', Referenced by : ' . $reference);
		}
		
		$code = file_get_contents($path);
		if ($code === false)
		{
			throw new CompileException('You may have no privilege to read this view:' . $path);
		}
	...
	private function parseUse($depth, $user_control, $param)
	{
		...
		$path = APP_PATH . '/use/' . str_replace('.', '/', $user_control) . $this->viewExt;
	
		if (!file_exists($path))
		{
			throw new CompileException('User Control file is not existed : ' . $path);
		}
		
		$code = file_get_contents($path);
		if ($code === false)
		{
			throw new CompileException('You may have no privilege to read this user control:' . $path);
		}
		$code = "{use $param}" . $this->preProcess($code, APP_PATH . '/view/', $depth) . '{/use}';
	...
	private function save()
	{
		...
		if ($suc === false)
		{
			throw new CompileException('Failed to write contents to cache file.');
		}
	}
	...
	private function preLoad()
	{
		...
		if (is_array($this->containers))
		{
			...
			foreach ($this->containers as $container)
			{
				...
				if (file_exists(APP_PATH . strtolower(DIRECTORY_SEPARATOR . 'container' . DIRECTORY_SEPARATOR . $class . '.php')))
				{
					$h = file_get_contents(APP_PATH . strtolower(DIRECTORY_SEPARATOR . 'container' . DIRECTORY_SEPARATOR . $class . '.php'));
				}
				elseif (file_exists(SYS_PATH . strtolower(DIRECTORY_SEPARATOR . 'container' . DIRECTORY_SEPARATOR . $class . '.php')))
				{
					$h = file_get_contents(SYS_PATH . strtolower(DIRECTORY_SEPARATOR . 'container' . DIRECTORY_SEPARATOR . $class . '.php'));
				}
				else
				{
					$h = '';
